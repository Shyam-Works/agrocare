...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (categoryMap.size > 0) {
            const categoryIds = Array.from(categoryMap.keys()).map(id => new mongoose.Types.ObjectId(id));
            const realCategories = await PlantCategory.find({ 
                _id: { $in: categoryIds } 
            }).lean();
            
            console.log("Found", realCategories.length, "categories in plant_categories collection");
        ...
        // P3: Disease Trend
        const groupFormat = (timeRange === '7d' || timeRange === '30d') ? '%Y-%m-%d' : '%Y-%m'; 
        const trendPipeline = [
            { $match: matchCriteria },
            {
                $group: {
                    _id: { $dateToString: { format: groupFormat, date: "$diagnosed_date" } },
                    diseased: { 
                        $sum: { 
                            $cond: [isHealthyCondition, 0, 1] 
                        } 
        ...
        const severityPipeline = [
            ...
            {
                $bucket: {
                    groupBy: "$severity_percentage",
                    boundaries: [0, 20, 50, 101],
                    default: "Severe",
                    output: { count: { $sum: 1 } }
                }
            },
            {
                $project: {
                    ...
                    level: {
                        $switch: {
                            branches: [
                                { case: { $lt: ["$_id", 20] }, then: "Mild" },
                                { case: { $lt: ["$_id", 50] }, then: "Moderate" },
                                { case: { $lt: ["$_id", 101] }, then: "Severe" },
                            ],
                            default: "Unknown"
                        }
                    },
        ...
        const activityPipeline = [
            ...
            { $group: { _id: { $dateToString: { format: "%Y-%m", date: "$diagnosed_date" } }, scans: { $sum: 1 } } },
        ...
        if (prevStats.totalDiagnoses > 0) {
            change = Math.round(((currentStats.totalDiagnoses - prevStats.totalDiagnoses) / prevStats.totalDiagnoses) * 100);
        ...
        const totalDiseased = distributionDataRaw.reduce((sum, item) => sum + item.value, 0);
        ...
        const diseaseDistribution = distributionDataRaw.map(item => {
            ...
            return {
                ...
                value: item.value,
                percentage: totalDiseased > 0 ? Math.round((item.value / totalDiseased) * 100) : 0,
            }
        });
        if (othersCount > 0) {
            const othersPercentage = totalDiseased > 0 ? Math.round((othersCount / totalDiseased) * 100) : 0;
            diseaseDistribution.push({ name: 'Others', value: othersCount, percentage: othersPercentage });
        }

        // Format severity data
        const severityMapping = { 'Mild': '#22c55e', 'Moderate': '#f59e0b', 'Severe': '#ef4444' };
        const severityData = Object.keys(severityMapping).map(level => {
            const dataPoint = severityDataRaw.find(d => d.level === level) || { count: 0 };
            return { level: level, count: dataPoint.count, color: severityMapping[level] };
        });

        // Format monthly scans
        const monthOrder = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        const monthlyScans = activityDataRaw.map(item => {
        ...
        const recentDiagnosesFormatted = recentDiagnosesRaw.map(item => {
            const isHealthy = !item.disease_name || 
                            item.disease_name.toLowerCase() === 'healthy';
            const severity = isHealthy ? 0 : (item.severity_percentage || 0);
            
            return {
                date: new Date(item.diagnosed_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
                category: item.category_name || 'Uncategorized',
                disease: isHealthy ? 'Healthy' : (item.disease_name || 'Unknown Disease'),
                severity: severity,
                status: isHealthy ? 'Healthy' : 
                        (severity > 50 ? 'Active' :
                        severity > 15 ? 'Monitoring' : 'Treated')
            };
        });

        console.log("\n========== SENDING RESPONSE ==========");
        console.log("Categories:", categoriesFormatted.length);
        console.log("Stats:", statsFormatted.map(s => `${s.title}: ${s.value}`));
        console.log("=========================================\n");
